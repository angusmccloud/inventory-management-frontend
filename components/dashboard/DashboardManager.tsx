...
import { listDashboards, deleteDashboard, rotateDashboard } from '@/lib/api/dashboards';
  ...
  ({ familyId: _familyId, onCreateNew, onEdit }, ref) => {
    ...
    const confirmRotateUrl = async (): Promise<void> => {
      ...
      try {
        ...
        showSnackbar({
          variant: 'success',
          text: `Link updated successfully! New URL: ${newUrl}`,
        });
        await loadDashboards();
      } catch (err) {
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
            {dashboards.map((dashboard) => (
              <Card key={dashboard.dashboardId} elevation="low" padding="md" interactive>
                <h3 className="mb-2 text-lg font-semibold text-text-primary">{dashboard.title}</h3>

                {/* Main content area with info on left and Copy Link button on right */}
                <div className="mb-3 flex gap-3">
                  {/* Left side: Info fields */}
                  <div className="flex-1 space-y-1 text-xs">
                    <Text variant="caption" color="secondary">
                      Type: {dashboard.type === 'location' ? 'Location-based' : 'Item-based'}
                    </Text>
                    <Text variant="caption" color="secondary">
                    ...
                    <Text variant="caption" color="secondary">
                      ...
                      {dashboard.lastAccessedAt
                        ? new Date(dashboard.lastAccessedAt).toLocaleDateString()
                        : 'Never'}
                    </Text>
                    <Text variant="caption" color="secondary">
                      Accessed: {dashboard.accessCount} times
                    </Text>
                  </div>

                  {/* Right side: Large Copy Link button */}
                  <Button
                    variant="secondary"
                    onClick={() =>
                      handleCopyUrl(
                        `${window.location.origin}/d/${dashboard.dashboardId}`,
                        dashboard.dashboardId
                      )
                    }
                    className="h-24 w-24 flex-shrink-0 !p-0"
      ...
      <div className="mx-auto max-w-7xl">
        ...
        {dialogState.type === 'rotate' && (
          <Dialog
            isOpen={true}
            type="primary"
            title="Update List Link"
            message="Updating this list link will deactivate the current URL and generate a new one. Anyone with the old link will lose access immediately until you share the new link."
